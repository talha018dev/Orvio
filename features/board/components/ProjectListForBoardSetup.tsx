...
import { getActiveProjectsListInfiniteQueryOptions } from '@/features/projects/queryOptions/projectQueryOptions';
...
const ProjectListForBoardSetup = () => {
    ...
    const { data, error, fetchNextPage, hasNextPage, isFetching, isLoading, isRefetching } =
        useInfiniteQuery(getActiveProjectsListInfiniteQueryOptions(pagination, 'Active'));
    ...
    return (
        <>
            ...
            <section className='grid grid-cols-2 gap-3 mt-3 pb-3'>
                {
                    !activeProjectsList?.length ? <EmptyListView /> :
                        activeProjectsList?.length ? activeProjectsList?.map((project, index) => {
                            const isLastElement = index === activeProjectsList.length - 1;
                            return (
                                <Link
                                    key={project?._id}
                                    href={project?.boardId ? `/board/${project?._id}/${project?.boardId}/details?projectName=${project?.name}` : `/board/${project?._id}/setup?projectName=${project?.name}`}
                                >
                                    <motion.div
...
export default ProjectListForBoardSetup