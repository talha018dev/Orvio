...
export default function ReactMultiSelect({ labelClassName = '', methods, label, name, listItems, placeholder, disabled, isLoading, createButtonClicked, labelOptional, useCustomOption = false }: PropTypes) {
        ...
        return (
            <components.Option {...props}>
                <div className="flex cursor-pointer items-center gap-2 relative">
                    ...
                    <div>
                        <div className="text-sm text-primaryText">{label}</div>
    ...
    return (
        <div className="pb-8">
            <label className={cn('w-full text-sm text-secondaryText', labelClassName)}>
                {label}<span className={cn('pl-[2px] text-secondaryText')}>{labelOptional ? '(optional)' : ''}</span>
            </label>
            <Controller
                name={name}
